...
const TEAM_PROJECTS: TeamProject[] = [
    ...
    comments: [
      {
        ...
        avatar: AVATAR_URLS[1],
        time: '1 day ago',
        text: 'Wireframes look great. Can we refine the checkout flow?',
        likes: 3,
      },
    ],
  },
  {
    ...
    tags: ['branding', 'logo'],
    detailAssets: [
      { id: 'a5', name: 'logo_final.svg', size: '120KB', type: 'image' },
    ],
    comments: [],
  },
  {
    ...
    tags: ['data', 'charts'],
    detailAssets: [
      { id: 'a6', name: 'dashboard_spec.pdf', size: '2MB', type: 'file' },
    ],
    comments: [],
  },
]
...
function StatusBadge({ status }: { status: ProjectStatus }) {
  const styles: Record<ProjectStatus, string> = {
    ...
    'In Progress': 'bg-neutral-100 text-neutral-600',
    Completed: 'bg-neutral-100 text-neutral-600',
  }
  return (
    <span className={`inline-flex items-center px-3 py-1 rounded-md text-[12px] font-medium ${styles[status]}`}>
      {status}
    </span>
  )
}
...
function ProjectCard({ project, onClick }: { project: TeamProject; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="bg-white border border-neutral-200 rounded-2xl p-5 flex flex-col gap-4 hover:shadow-md transition-shadow cursor-pointer"
    >
...
function ProjectListRow({ project, onClick }: { project: TeamProject; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="bg-white border border-neutral-200 rounded-xl px-5 py-4 flex items-center gap-5 hover:shadow-md transition-shadow cursor-pointer"
    >
...
<div className="flex flex-col gap-6 max-w-[860px] mx-auto w-full">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-1 text-[13px] font-medium text-neutral-500 hover:text-neutral-900 transition-colors w-fit"
      >
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl px-7 py-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* Meta row: due, priority, members, tags */}
            <div className="flex items-center gap-4 flex-wrap">
              <span className="text-[13px] text-neutral-500">
                Due: {project.dueDate.replace('Due ', '')}
              </span>
              {project.priority && (
                <span className="text-[13px] text-neutral-500">
                  Priority: <span className="font-semibold text-neutral-800 capitalize">{priorityLabel[project.priority]}</span>
                </span>
              )}
              {/* Member avatars */}
              ...
              {project.tags?.map((tag) => (
                <span key={tag} className="px-3 py-1 rounded-lg border border-neutral-200 bg-neutral-50 text-[12px] font-medium text-neutral-600">
                  {tag}
                </span>
              ))}
            </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-3">
          {(project.detailAssets ?? []).map((asset) => (
            <div
              key={asset.id}
              className="flex items-center gap-3 bg-white border border-neutral-200 rounded-xl px-4 py-3 hover:shadow-sm transition-shadow cursor-pointer group min-w-[200px]"
            >
      ...
      <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                  ...
                  {comment.attachedAsset && (
                    <div className="flex items-center gap-2.5 mt-3 bg-white border border-neutral-200 rounded-xl px-3 py-2.5 w-fit group cursor-pointer hover:shadow-sm transition-shadow">
                      <div className="w-7 h-7 rounded-md bg-neutral-100 flex items-center justify-center flex-shrink-0">
                        <svg className="w-3.5 h-3.5 text-neutral-500" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                          <polygon points="5 3 19 12 5 21 5 3" />
                        </svg>
                      </div>
                      ...
                      <button className="opacity-0 group-hover:opacity-100 ml-1 p-0.5 rounded hover:bg-neutral-100 text-neutral-400 transition-opacity">
                ...
                <div className="flex items-center gap-3 pl-1">
                  ...
                  <button
                    ...
                    className="text-[12px] font-medium text-neutral-400 hover:text-neutral-700 underline underline-offset-2 transition-colors"
                  >
                    ...
                    {comment.replies.map((reply) => (
                      <div key={reply.id} className="flex gap-2.5">
                        ...
                        <div className="flex flex-col gap-1.5 flex-1">
                          ...
                          <div className="flex items-center gap-3 pl-1">
                            <button className="flex items-center gap-1 text-[11px] text-neutral-400 hover:text-neutral-700 transition-colors">
                              <span>👍</span>
                              <span className="font-medium">{reply.likes}</span>
                            </button>
                            <button className="text-[11px] font-medium text-neutral-400 hover:text-neutral-700 underline underline-offset-2 transition-colors">
                              Reply
                            </button>
                          </div>
                        </div>
                ...
                {replyingTo === comment.id && (
                  <div className="flex items-center gap-2 pl-4">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-lg bg-neutral-900 hover:bg-neutral-700 text-white flex items-center justify-center transition-colors"
                    >
        ...
        {/* No comments state */}
        {comments.length === 0 && (
          <div className="text-center py-8 text-[13px] text-neutral-300">
      ...
      <div className="flex items-center gap-3 bg-white border border-neutral-200 rounded-2xl px-4 py-3 sticky bottom-4 shadow-lg">
        ...
        <input
          ...
          className="flex-1 text-[13px] text-neutral-700 placeholder:text-neutral-300 focus:outline-none bg-transparent"
        />
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-neutral-900 hover:bg-neutral-700 text-white rounded-xl px-4 py-1.5 text-[12px] font-semibold transition-colors"
          >
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={sortRef}>
            <button
              ...
              className="flex items-center gap-2 bg-white border border-neutral-200 rounded-lg px-3 py-2 text-[12px] font-medium text-neutral-600 hover:bg-neutral-50 shadow-2xs transition-colors"
            >
              Sort
            ...
            {sortOpen && (
              <div className="absolute right-0 mt-1.5 w-40 bg-white border border-neutral-200 rounded-xl shadow-xl p-1.5 z-20">
                {['Name', 'Due Date', 'Status', 'Progress'].map((opt) => (
                  <button key={opt} className="w-full text-left px-3 py-2 text-[12px] font-medium text-neutral-600 hover:bg-neutral-50 rounded-lg transition-colors">
                    {opt}
                  </button>
                ))}
              </div>
          ...
          <div className="flex items-center bg-white border border-neutral-200 rounded-lg overflow-hidden shadow-2xs">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium transition-colors ${
                displayMode === 'list' ? 'bg-neutral-900 text-white' : 'text-neutral-500 hover:bg-neutral-50'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium transition-colors border-l border-neutral-200 ${
                displayMode === 'grid' ? 'bg-neutral-900 text-white' : 'text-neutral-500 hover:bg-neutral-50'
              }`}
            >
      ...
      {displayMode === 'grid' ? (
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((project) => (
            <ProjectListRow key={project.id} project={project} onClick={() => onSelectProject(project)} />
          ))}
        </div>
      )}
